    ...
    if len(result) == 0:
        ...
        with pg.BusyCursor():
            ...
            if len(new_imgs) == len(new_masks) > 0:
                ...
                win.setWindowTitle('{}'.format(path_in)) 
            else:
...
def cell_tracking_clicked(param):
    ...
    with pg.BusyCursor():
        ...
        trj, col_tuple, col_weights = track_cells(path_in, merged_masks, 5, min_cell_id=1)
        ...
        for i in range(len(cell_ids)):
            if cell_ids[i] != 0:
                vis_frames = sorted(initial_trj[initial_trj.particle == cell_ids[i]]["frame"])
                miss_frames = sorted(find_missing(vis_frames))
                reference_frame = [0]*len(miss_frames)
                for j in range(len(miss_frames)):
                    temp_fwd_frame = min(k for k in vis_frames if k > miss_frames[j])
                    temp_bwd_frame = max(k for k in vis_frames if k < miss_frames[j])
                    temp_fwd_area = int(initial_trj[(initial_trj.particle == cell_ids[i]) & (initial_trj.frame == temp_fwd_frame)]["area"])
                    temp_bwd_area = int(initial_trj[(initial_trj.particle == cell_ids[i]) & (initial_trj.frame == temp_bwd_frame)]["area"])
                    if temp_fwd_area > temp_bwd_area:
                        reference_frame[j] = temp_fwd_frame
                    else:
                        reference_frame[j] = temp_bwd_frame
                for j in range(len(miss_frames)):
                    current_miss_frame = id_masks[:,:,miss_frames[j]].astype(np.uint8)
                    overlay_frame = id_masks[:,:,reference_frame[j]].astype(np.uint8)
                    new_frame = np.where(overlay_frame != cell_ids[i], current_miss_frame, overlay_frame)
                    id_masks[:,:,miss_frames[j]] = new_frame

        trj, col_tuple, col_weights = track_cells(path_in, id_masks.astype(np.uint8), 0, min_cell_id=1)
        initial_trj = trj.copy()
        id_masks, cell_ids, color_list, background_id = prepare_mask_colors(id_masks.astype(np.uint8), trj)
        cell_color_idx = dict([(x, col_idx) for col_idx, x in enumerate(cell_ids)])
        id_masks_initial = id_masks.copy()
        ...
        logging.info('================= Visualizing cell contours and tracks =================')
